...
import { getBuildVersionNumber } from '../src/VersionNumber';
...
test('getBuildVersionNumber throws error when regex does not match', async () => {
  ...
  await expect(getBuildVersionNumber('file.txt', 'invalid-regex')).rejects.toThrow(
...
test('getBuildVersionNumber returns build version number', async () => {
  ...
  const buildVersionNumber = await getBuildVersionNumber('file.txt', 'version: (\\d+\\.\\d+\\.\\d+)');
