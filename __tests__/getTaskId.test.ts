...
describe('getTaskId', () => {
  ...
  test('should throw an error if the branch name is not found', async () => {
    await expect(getTaskId()).rejects.toThrow('Branch name is not found, is this being run in a pull request?');
  });
  ...
  test('should debug the task ID if it is found in the branch name', async () => {
    ...
    await getTaskId();
    expect(core.debug).toHaveBeenCalledWith('Found task id 12345 w');
  });
  ...
  test('should debug a message if the task ID is not found in the branch name', async () => {
    ...
    jest.spyOn(core, 'debug').mockImplementation(() => {});
    await getTaskId();
    expect(core.debug).toHaveBeenCalledWith('Unable to find taskId in branch name test/bug_fix');
  });
});
